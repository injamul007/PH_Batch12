export default async function StoryDetails({ params }) {
  ...
  const story = devStories.find((story) => story.id === Number(id));
  // console.log(story);
  if (!story) {
    return <h2>Story Not Found</h2>;
  }
  return (
    <div className="max-w-3xl mx-auto bg-gray-200 shadow-md rounded-lg p-6 border border-gray-200">
      {/* Top Section */}
      <div className="flex items-center gap-5">
        ...
        />
        <div>
          <h1 className="text-2xl font-semibold text-gray-500">{story.name}</h1>
      ...
      {/* Story */}
      <div className="mt-6">
        <h2 className="text-xl font-semibold mb-2 text-gray-500">Learning Story</h2>
        <p className="text-gray-700 leading-relaxed">{story.story}</p>
      </div>

      {/* About */}
      <div className="mt-6">
        <h2 className="text-xl font-semibold mb-2 text-gray-500">About Developer</h2>
        <p className="text-gray-700 leading-relaxed">{story.about}</p>
      </div>

      {/* Skills */}
      <div className="mt-6">
        <h2 className="text-xl font-semibold mb-2 text-gray-500">Skills</h2>
        <div className="flex flex-wrap gap-2">
          {story.skills.map((skill, index) => (
            <span
              key={index}
              className="bg-gray-200 px-3 py-1 rounded-full text-sm text-gray-500"
            >
              {skill}
            </span>
          ))}
