...
app.get("/apps", async (req, res) => {
  try {
    ...
    const sortOption = {}
    if(sort) {
      sortOption[sort || null] = (order === 'asc') ? 1 : -1
    }
    ...
    const apps = await appsCollection.find(query).limit(Number(limit)).skip(Number(skip)).sort(sortOption).project({description:0,ratings:0}).toArray();
